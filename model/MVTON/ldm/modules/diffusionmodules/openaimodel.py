...
from model.MVTON.ldm.modules.diffusionmodules.util import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
...
from model.MVTON.ldm.modules.attention import SpatialTransformer
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, down_samples=None, mid_samples=None, y=None,**kwargs):
        """
        ...
        # print("---len={}-------".format(len(self.output_blocks)))
        for i, module in enumerate(self.output_blocks):
            h = th.cat([h, hs.pop()], dim=1)
            # -------------------------------------------
            if i > 2 and down_samples is not None:
                context = th.cat([context, down_samples[11-i]], dim=1)
            # -------------------------------------------
            h = module(h, emb, context)
        h = h.type(x.dtype)
        if self.predict_codebook_ids:
